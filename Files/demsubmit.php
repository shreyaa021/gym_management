...
$query="insert into users(userid,Full_name,username,gender,mobile,email,dob,joining_date,pass_key,securekey) values('$memID','$uname','$uname','$gender','$phn','$email','$dob','$jdate','$password','$uname')";
          ...
          if(mysqli_query($con,$query2)==1){
            ...
            if(mysqli_query($con,$query4)==1){
              ...
              if(mysqli_query($con,$query5)==1){
               ...
               echo "<head><script>alert('Member Adding Failed');</script></head></html>";
              echo "error: ".mysqli_error($con);
               //Deleting record of users if inserting to enrolls_to table failed to execute
                $query3 = "DELETE FROM users WHERE userid='$memID'";
                mysqli_query($con,$query3);
